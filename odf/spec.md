...
<Diagram src="/images/pages/spec/transform.svg" alt="Diagram: Transformation"/>
...
<Diagram src="/images/pages/spec/dataset.svg" alt="Diagram: Dataset/Data"/>
...
<Diagram src="/images/pages/spec/metadata.svg" alt="Diagram: Data Slices and Metadata"/>
...
<Diagram src="/images/pages/spec/metadata-chain.svg" alt="Diagram: Metadata Chain"/>
...
<Diagram src="/images/pages/spec/metadata-chain-2.svg" alt="Diagram: Dataset as a Content-Addressable Graph"/>
...
<Diagram src="/images/pages/spec/dataset_graph.svg" alt="Diagram: Dataset Graph"/>
...
<Diagram src="/images/pages/spec/engine-execution-env.svg" alt="Diagram: Derivative Transformation"/>
...
<Diagram src="/images/pages/spec/watermarks_in_stream.svg" alt="Diagram: Watermarks in the Stream"/>
...
<Diagram src="/images/pages/spec/watermarks_vs_time.svg" alt="Diagram: Watermarks in Time Domains"/>
...
## Manifest
When [Metadata](#metadata) objects are saved on disk or transmitted on the network the typically wrapped in an extra <Schema t="Manifest" id="manifest"/> layer.
...
The resulting DID is stored in the first [MetadataBlock](#metadata-chain) in the chain of every [Dataset](#dataset), called <Schema t="Seed" id="seed"/>.
...
See also:
- [RFC-003: Content Addressability](/odf/rfcs/003-content-addressability.md)
- [RFC-012: Recommend base16 encoding](/odf/rfcs/012-recommend-base16-encoding.md)
...
See also:
- [RFC-015: Unified Changelog Stream Schema](/odf/rfcs/015-unified-changelog-stream-schema.md)
...
<Diagram src="/images/pages/spec/dataset_layout.svg" alt="Diagram: Dataset Layout"/>
...
<Diagram src="/images/pages/spec/engine_transform.svg" alt="Sequence Diagram: Execute Query"/>
...
See also:
- [RFC-002](/odf/rfcs/002-logical-data-hashes.md)
- [RFC-012](/odf/rfcs/012-recommend-base16-encoding.md)
...
1. The <Schema t="MetadataBlock" id="metadatablock"/> is serialized into [FlatBuffers](https://google.github.io/flatbuffers/) format following a two-step process to ensure that all variable-size buffers are laid out in memory in a consistent order:
...
2. Block content is then nested into a <Schema t="Manifest" id="manifest"/> object using same serialization rules as above
3. The resulting `flatbuffer` data is fed into [SHA3-256](https://en.wikipedia.org/wiki/SHA-3) digest algorithm.

For use in the <Schema t="Manifest" id="manifest"/>'s `kind` field the `multicodec` table is extended with the following codes in the "private use area":
...
3) Data part files can be downloaded by using <Schema t="DataSlice::physicalHash" id="dataslice"/> links with `GET /data/{physicalHash}`
4) Checkpoints are similarly downloaded using <Schema t="Checkpoint::physicalHash" id="checkpoint"/> links with `GET /checkpoints/{physicalHash}`
...
See also:
- [RFC-007: Simple Transfer Protocol](/odf/rfcs/007-simple-transfer-protocol.md)
...
The detailed format of endpoints is described here: [Smart Transfer Protocol: OpenAPI](https://github.com/open-data-fabric/open-data-fabric/blob/master/protocols/smart-transfer-protocol.openapi.yaml).
...
The details of the individual asynchronous messages are specified here: [Smart Transfer Protocol: AsyncAPI](https://github.com/open-data-fabric/open-data-fabric/blob/master/protocols/smart-transfer-protocol.asyncapi.yaml).

See also:
- [RFC-008: Smart Transfer Protocol](/odf/rfcs/008-smart-transfer-protocol.md)
