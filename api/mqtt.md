...
Kamu Node supports ingesting data using the [MQTT](https://mqtt.org/) protocol. See <Schema t="FetchStep::Mqtt"/> and this [example](https://github.com/kamu-data/kamu-cli/tree/master/examples/mqtt).
...
Kamu Node may not have perfect connectivity with MQTT broker, so prolonged periods of disconnects should be accounted for. Additionally, MQTT sources can be used even with [kamu-cli](/cli) tool, and since the tool doesn't use any background processes it will connect to MQTT broker only when you run:
